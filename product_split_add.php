...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        // 1. ตรวจสอบและล็อคสต็อกสินค้าต้นทาง (ดึงข้อมูลที่จำเป็นมาด้วย)
        $check_stmt = $conn->prepare("SELECT stock_quantity, product_name, category_id, image_path FROM products WHERE product_id = ? FOR UPDATE");
        ...
        if ($new_product_type === 'existing') {
           ...
           $new_product_price = (float)$_POST['new_product_price'];
            ...
            // 3. สร้างสินค้าใหม่
            $create_stmt = $conn->prepare("INSERT INTO products (product_name, stock_quantity, product_unit, category_id, selling_price, reorder_level, image_path) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $create_stmt->bind_param("sdsidis", $new_product_name, $new_qty, $new_product_unit, $parent_product['category_id'],  $new_product_price, $new_reorder_point, $parent_product['image_path']);
            $create_stmt->execute();
        ...
        // 2. ลดสต็อกสินค้าต้นทาง (ทำหลังจากตรวจสอบเงื่อนไขทั้งหมดแล้ว)
        ...
        // 3. เพิ่มสต็อกสินค้าใหม่ (กรณีเลือกสินค้าที่มีอยู่แล้ว)
        if ($new_product_type === 'existing') {
    ...
    $(document).ready(function() {
        ...
        $('input[name="new_product_type"]').on('change', function() {
            if (this.value === 'new') {
                ...
                $('#existing_product_fields').find('select, input').prop('disabled', true).prop('required', false);
                
                $('#new_product_fields').show();
                $('#new_product_fields').find('input').prop('disabled', false).prop('required', true);

            } else { // existing
                $('#new_product_fields').hide();
                $('#new_product_fields').find('input').prop('disabled', true).prop('required', false);
